...
async def on_message_delete(message):
  ...
  if message.author.bot:
    return
  console.info(f"{message.author.name} deleted a message in {message.guild.name}.\nMessage: {message.content}")
...
async def on_message_edit(before, after):
  ...
  console.info(f"{before.author.name} edited a message in {before.guild.name}.\nBefore: {before.content}\nAfter: {after.content}")
...
async def feedback(interaction: Interaction, *, feedback: str):
  server = bot.get_guild(937841015648292934)
  ticket_id = random.randint(1, 10000)
  if ticket_id in tickets:
    ticket_id = random.randint(1, 10000)
  else:
    tickets[ticket_id] = interaction.user.id
  tickets[ticket_id] = interaction.user.id
  channel = await server.create_text_channel(f"feedback-{ticket_id}")
  del tickets[ticket_id]
  tickets[ticket_id] = {interaction.user.id : channel.id}
  print(tickets)
  created = interaction.created_at.strftime("%d/%m/%Y %H:%M")
  bingus_caretaker = server.get_role(943341465424232468)
  await channel.set_permissions(server.default_role, send_messages=False, read_messages=False)
  await channel.set_permissions(bingus_caretaker, send_messages=True, read_messages=True)
  await channel.move(beginning=True, category=bot.get_channel(1076698164712714321))
  embed = nextcord.Embed(title="Feedback", description=f"**Feedback:** {feedback}", color=nextcord.Color.random())
  embed.set_footer(text=f"Submitted at {created} | Ticket ID: {ticket_id}")
  embed.set_author(name=interaction.user.name, icon_url=interaction.user.avatar)
  await channel.send("<@164061868741099520>",embed=embed)
  await interaction.send("Your feedback has been sent to the bot owner!", ephemeral=True)
  # for i in owners.values:
  #    owner = bot.get_user()
  #    await owner.send("you got a new ticket")
