...
class AutoResponse(commands.Cog):
  ...
  async def on_message(self, message):
    ...
    if msg.lower() == cock.values() and await func.B_RandomChance(50) == True:
      print("Rolled")
    ...
    response = {
      ...
      "I get bitches": ["your sister doesn't count, unfortunately", "send user hoe count", "Didn't I just see a video of you in bed with another dude???", ":billed_cap:", "good one :joy:", "Anime waifus don't count, sorry to say", f"No {sender.name}, your hand doesn't count.", "You know your minecraft girlfriend spawned in your bestfriends house, right?"],
    ...
    for say in list(response.keys()):
      if say.lower() in msg.lower():
        ...
        if reply == 'send user hoe count':
          embed = nextcord.Embed(name=f"A list of {sender.name}'s bitches")
          embed.add_field(text="*Unsurprisingly this list is empty*", value="\n")
          await message.reply(embed=embed)
        else:
