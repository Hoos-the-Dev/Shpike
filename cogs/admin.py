...
class Admin(commands.Cog):
  ...
  async def on_message(self, message):
    if message.author == self.bot.user: pass
    if self.bot.user.mention in message.content:
      if str(message.content).startswith(f"{self.bot.user.mention} ban"):
       user = str(message.content).removeprefix(f"{self.bot.user.mention} ban")
       user = nextcord.Member(user)
       print(user)
