...
async def play_next(self, ctx: commands.Context):
    await Song.process(self, ctx)


class Youtube:
    def __init__(self, bot):
        self.song_queue = song_queue
        self.requeste = requeste

    async def play(ctx: commands.Context, song: wavelink.YouTubeTrack):
        if len(song_queue) == 0:
...
class Spotify:
    ...
    async def plate(ctx: commands.Context):
        if len(song_queue) == 0:
            embed = Embed(title="That's all folks", description="The queue has finished playing", color=nextcord.Color.red())
            print("Queue finished playing")
            await ctx.send(embed=embed)
...
class Song:
    ...
    async def process(self, ctx: commands.Context):
        ...
        if str(song_queue[0]).startswith("https://www.youtube.com/") or str(song_queue[0]).startswith("https://www.youtu.be/"):
           await Youtube.play(ctx, song=song_queue[0])
        else:   
            try: await Spotify.plate(ctx)
            except Exception as e:
                print(e)
                Song.update_queue()
                await Song.process(ctx)
