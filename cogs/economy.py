...
class Economy(commands.Cog):
  ...
  async def deposit(self, ctx: commands.Context, amount):
    ...
    if amount == "all" or amount == "max":
      ...
      if bank >= maxbank:
        ...
        return
      if wallet + bank > maxbank:
        #deposit until bank is full
        sub_amount = await prevent_overflow(self, ctx.author, int(bank))
        amount = wallet - sub_amount
    ...
    try: amount = int(amount)
    except: await ctx.reply("NUMBERS, I NEED NUMBERS"); return
    if amount > wallet:
  ...
  async def beg(self, ctx: commands.Context):
    ...
    if wallet >= 100000:
      ...
      await ctx.send(embed=embed)
      return
    if await B_RandomChance(0.89):
      stolen = randint(1, wallet)
      if stolen == wallet:
        embed = Embed(title="Jesus...", description=f"{citizen_firstnames} {citizen_lastnames} beats you up and takes all of your money", color=Color.dark_red())
        embed.set_footer(f"- ${wallet}")
        await ctx.send(embed=embed)
        await update_balance(self, ctx.author, wallet)
        return
      embed = Embed(title="Jesus...", description=f"{citizen_firstnames} {citizen_lastnames} beats you up and takes a portion of your money")
      embed.set_footer(f"- ${stolen}")
      await ctx.send(embed=embed)
      await update_balance(self, ctx.author, stolen)
      return
  ...
  "Ok, now smile for the camera!"
    ...
    if randint(1, 10000) == randint(1, 10000):
      embed = Embed(title="Fuck X but <3 you Elon", description=f"Elon Musk gave you $1,000,000", color=Color.gold())
      elon_responses = [
  "Sure, why not? Here's $1,000,000.",
  "No problem, happy to help. Make good use of it.",
  "i paid 40 billion dollars for twitt- i mean X, i can afford to give you 1 million dollars",
  ...
  "life is too short to not spread happiness. take this $1,000,000 and do something great with it."]
    ...
    if await B_RandomChance(5):
      embed = Embed(title="Oh whats this?", description=f"You found a wallet with $100 in it", color=Color.dark_gold())
