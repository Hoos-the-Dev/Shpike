...
class Owner(commands.Cog):
  ...
  async def spawn_cash(self, ctx: commands.Context, amount:int, user: nextcord.Member = None):
    ...
    embed= B_CreateEmbed(None, f"Added {amount} to {user.name}'s balance", color=nextcord.Color.yellow()) 
    await ctx.send(embed=embed)
  
  @command(description= "mess around with someones bank balance >:D")
  async def change_bank(self, ctx: commands.Context, amount:int, user: nextcord.Member = None):
    self.db = await aiosqlite.connect("resources/data/bank.db")
    if B_IsMemberOwner(self, ctx.author) == False:
      await ctx.send('smh, trying to inflate the economy')
      return
    ...
    if user is None:
      user = ctx.author
    await set_bank(self, user, amount)
    embed = B_CreateEmbed(None, f"Set {user.name}'s bank balance to {amount}", color=nextcord.Color.yellow())
    await ctx.send(embed=embed)

  @command(description= "mess around with someones bank balance >:D")
  async def change_banklimit(self, ctx: commands.Context, amount:int, user: nextcord.Member = None):
    self.db = await aiosqlite.connect("resources/data/bank.db")
    if B_IsMemberOwner(self, ctx.author) == False:
      await ctx.send('smh, trying to inflate the economy')
      return
    ...
    if user is None:
      user = ctx.author
    await set_maxbank(self, user, amount)
    embed = B_CreateEmbed("", f"Set {user.name}'s bank limit balance to {amount}", color=nextcord.Color.yellow())
    await ctx.send(embed=embed)   
