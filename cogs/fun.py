...
class Fun(commands.Cog):
  ...
  async def rps(self, interaction:Interaction, opponent: nextcord.User=SlashOption(name="opponent", description="The user you're playing against", required=False), wager:str=SlashOption(name="wager", description="The amount you're wagering", required=False)):
    if opponent.id == interaction.user.id: await interaction.send("You can't play with yourself"); return
    if opponent.bot: await interaction.send("You can't play with a bot"); return
    if wager == None: wager = 0
    if wager == "all":
      wager = await get_balance(self, interaction.user)
      wager = wager[0]
    if wager == "half":
      wager = await get_balance(self, interaction.user)
      wager = wager[0]
      wager = wager/2
    if "b" in wager:
      wager = wager.replace("b", "")
      wager = int(wager)
      wager = wager*1000000000
    if "m" in wager:
      wager = wager.replace("m", "")
      wager = int(wager)
      wager = wager*1000000
    if "k" in wager:
      wager = wager.replace("k", "")
      wager = int(wager)
      wager = wager*1000
