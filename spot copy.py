...
class Music(commands.Cog):
    ...
    async def play(self, ctx: commands.Context, *, url):
        ...
        try:
            if not vc.is_playing():
                await Song.add_to_queue(ctx.author.guild.id, url, author)
                print(song_queue)
    ...
    async def queue(self, ctx: commands.Context):
        try:
            ...
            song, requester, num = await Song.get_queue(serverid=ctx.author.guild.id)
            for i in range(len(song_queue[ctx.author.guild.id])):
                embed.add_field(name=f"{num}. {song}", value=f"Requested by {requester}", inline=False)
            await ctx.send(embed=embed)
        except KeyError:
